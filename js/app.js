...
// object to hold the various scales we will be using
var Dimensions = function(data, options) {
        options = options || {};
    ...
    return _.map(data, function(workout) {
        ...
        exercises = _.map(exercises, function(exercise, key) {
            return {
                ...
                totalReps: _.reduce(exercise, function(numReps, set) {
                    return numReps + parseInt(set.num) * parseInt(set.reps);
                }, 0)
            };
        });
...
var bindEvents = function() {
    $("rect").on("mouseover", function(event) {
        var hover = $("#hover-box");
        hover.text($(this).attr('hover-text'));
        hover.css({
            top: event.clientY,
            left: event.clientX,
            display: "block"
...
$(document).ready(function() {
    ...
    svg.selectAll(".date")
        ...
        .enter()
        .append("text")
        .attr("x", function(d, i) { return dimensions.time(d); })
        .attr("y", baseLine + 20)
        .attr("fill", "#444")
        .text(tickFormat);
    ...
    svg.selectAll(".bar-group")
        ...
        .attr("height", 0)
        .attr("x", function(d, i) { return dimensions.time(d.date) + rectWidth * i; })
        .attr("y", baseLine)
        .attr("hover-text", function(d) {
            var date = d.date;
